...
class RecoverAwsDDBNewSourceConfig(BaseModel):
    """
    Specifies the new destination Source configuration where the DynamoDB tables will be recovered.
    """ # noqa: E501
    region: Optional[RecoveryObjectIdentifier] = Field(description="Specifies the AWS region in which to deploy the Dynamo DB table.")
    source: Optional[RecoveryObjectIdentifier] = Field(description="Specifies the id of the parent source to recover the Dynamo DB table.")
    __properties: ClassVar[List[str]] = ["region", "source"]
