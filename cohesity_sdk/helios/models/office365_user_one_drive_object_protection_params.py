...
class Office365UserOneDriveObjectProtectionParams(BaseModel):
    ...
    exclude_folders: Optional[List[StrictStr]] = Field(default=None, description="Specifies filters to match OneDrive folders which should be excluded when backing up office365 onedrive source. Two kinds of filters are supported. a) prefix which always starts with '/'. b) posix which always starts with empty quotes(''). Regular expressions are not supported. If not specified, all the OneDrive will be protected.", alias="excludeFolders")
    ...
    __properties: ClassVar[List[str]] = ["indexingPolicy", "objects", "sourceId", "sourceName", "excludeFolders", "preservationHoldLibraryParams"]
