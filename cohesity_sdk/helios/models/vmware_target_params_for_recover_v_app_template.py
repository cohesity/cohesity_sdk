...
class VmwareTargetParamsForRecoverVAppTemplate(BaseModel):
    ...
    recovery_target_config: Optional[VmwareVAppTemplateRecoveryTargetConfig] = Field(default=None, description="Specifies the recovery target configuration if recovery has to be done to a different location which is different from original source or to original Source with different configuration. If not specified, then the recovery of the vApp templates will be performed to original location with all configuration parameters retained.", alias="recoveryTargetConfig")
    rename_recovered_v_app_template_params: Optional[RecoveredOrClonedVmsRenameConfig] = Field(default=None, description="Specifies params to rename the vApps templates that are recovered. If not specified, the original names of the vApp templates are preserved.", alias="renameRecoveredVAppTemplateParams")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if rename_recovered_v_app_template_params (nullable) is None
        # and model_fields_set contains the field
        if self.rename_recovered_v_app_template_params is None and "rename_recovered_v_app_template_params" in self.model_fields_set:
            _dict['renameRecoveredVAppTemplateParams'] = None
