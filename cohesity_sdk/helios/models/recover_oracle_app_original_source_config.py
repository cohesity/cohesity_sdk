...
class RecoverOracleAppOriginalSourceConfig(BaseModel):
    """
    Specifies the additional Source configuration parameters when databases will be recovered to original location.
    """ # noqa: E501
    attempt_complete_recovery: Optional[StrictBool] = Field(default=None, description="Whether or not this is a complete recovery attempt.", alias="attemptCompleteRecovery")
    roll_forward_log_path_vec: Optional[List[StrictStr]] = Field(default=None, description="List of archive logs to apply on Database after overwrite restore.", alias="rollForwardLogPathVec")
    roll_forward_time_msecs: Optional[StrictInt] = Field(default=None, description="UTC time in msecs till which we have to roll-forward the database.", alias="rollForwardTimeMsecs")
    stop_active_passive: Optional[StrictBool] = Field(default=None, description="Specifies whether allowed to automatically stop active passive resource.", alias="stopActivePassive")
