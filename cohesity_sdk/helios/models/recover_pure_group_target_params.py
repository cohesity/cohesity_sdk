...
class RecoverPureGroupTargetParams(BaseModel):
    ...
    """ # noqa: E501
    new_source_config: Optional[RecoverPureSanGroupNewSourceConfig] = Field(default=None, description="Specifies the new destination Source configuration parameters where the Pure group will be recovered. This is mandatory if recoverToNewSource is set to true.", alias="newSourceConfig")
    original_source_config: Optional[RecoverPureSanGroupOriginalSourceConfig] = Field(default=None, description="Specifies the Source configuration if Pure group is being recovered to Original Source. If not specified, all the configuration parameters will be retained.", alias="originalSourceConfig")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "newSourceConfig": RecoverPureSanGroupNewSourceConfig.from_dict(obj["newSourceConfig"]) if obj.get("newSourceConfig") is not None else None,
            "originalSourceConfig": RecoverPureSanGroupOriginalSourceConfig.from_dict(obj["originalSourceConfig"]) if obj.get("originalSourceConfig") is not None else None,
