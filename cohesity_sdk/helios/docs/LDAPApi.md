...
try:
	# Get LDAP connection status.
	api_response = client.ldap.get_ldap_connection_status(id, access_cluster_id=access_cluster_id, region_id=region_id)
	pprint(api_response)
except ApiException as e:
	print("Exception when calling LDAPApi->get_ldap_connection_status: %s\n" % e)
```
...
try:
	# Get Groups.
	api_response = client.ldap.get_ldaps(access_cluster_id=access_cluster_id, region_id=region_id, ids=ids, tenant_ids=tenant_ids, include_tenants=include_tenants)
	pprint(api_response)
except ApiException as e:
