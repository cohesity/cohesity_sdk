...
class AgentUpgradeTaskActionRequest(BaseModel):
    """
    Specifies the params to perform an action on an agent upgrade task.
    """ # noqa: E501
    action: Optional[StrictStr] = Field(default=None, description="Specifies the action type.<br> 'Cancel' indicates that the upgrade task needs to be cancelled.")
    ...
    def action_validate_enum(cls, value):
        ...
        if value not in set(['Retry', 'Cancel']):
            raise ValueError("must be one of enum values ('Retry', 'Cancel')")
        return value
