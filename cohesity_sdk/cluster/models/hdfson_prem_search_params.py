...
class HDFSOnPremSearchParams(BaseModel):
    """
    Parameters required to search HDFS on a cluster.
    """ # noqa: E501
    hdfs_types: List[StrictStr] = Field(description="Specifies types as Folders or Files or both to be searched.", alias="hdfsTypes")
    search_string: Optional[StrictStr] = Field(description="Specifies the search string to search the HDFS Folders and Files.", alias="searchString")
    ...
    __properties: ClassVar[List[str]] = ["hdfsTypes", "searchString", "sourceIds"]

    @field_validator('hdfs_types')
    def hdfs_types_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['HDFSFolders', 'HDFSFiles']):
                raise ValueError("each list item must be one of ('HDFSFolders', 'HDFSFiles')")
        return value
