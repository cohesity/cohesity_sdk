...
class HbaseOnPremSearchParams(BaseModel):
    """
    Parameters required to search Hbase on a cluster.
    """ # noqa: E501
    hbase_object_types: List[StrictStr] = Field(description="Specifies one or more Hbase object types be searched.", alias="hbaseObjectTypes")
    search_string: Optional[StrictStr] = Field(description="Specifies the search string to search the Hbase Objects", alias="searchString")
    ...
    __properties: ClassVar[List[str]] = ["hbaseObjectTypes", "searchString", "sourceIds"]

    @field_validator('hbase_object_types')
    def hbase_object_types_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['HbaseNamespaces', 'HbaseTables']):
                raise ValueError("each list item must be one of ('HbaseNamespaces', 'HbaseTables')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "hbaseObjectTypes": obj.get("hbaseObjectTypes"),
