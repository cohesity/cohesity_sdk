...
class NutanixFSObjectProtectionRequestParams(BaseModel):
    """
    ...
    backup_existing_snapshot: Optional[StrictBool] = Field(default=None, description="Specifies that snapshot label is not set for Data-Protect NutanixFS Volumes backup. If field is set to true, existing oldest snapshot is used for backup and subsequent incremental will be selected in ascending order of snapshot create time on the source. If snapshot label is set, this field is set to false.", alias="backupExistingSnapshot")
    ...
    __properties: ClassVar[List[str]] = ["continueOnError", "encryptionEnabled", "fileFilters", "fileLockConfig", "indexingPolicy", "prePostScript", "throttlingConfig", "backupExistingSnapshot", "continuousSnapshots", "excludeObjectIds", "nfsVersionPreference", "protocol", "snapshotLabel", "objects"]
