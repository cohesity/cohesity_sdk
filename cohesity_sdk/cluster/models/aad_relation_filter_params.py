...
class AadRelationFilterParams(BaseModel):
    """
    Determines filter that can be applied for a Aad node edge.
    """ # noqa: E501
    relation_type: Optional[StrictStr] = Field(default=None, description="Filters the edges which matches with specified node relation type.", alias="relationType")
    __properties: ClassVar[List[str]] = ["relationType"]
    ...
    def relation_type_validate_enum(cls, value):
        ...
        if value not in set(['Group', 'Membership']):
            raise ValueError("must be one of enum values ('Group', 'Membership')")
        return value
