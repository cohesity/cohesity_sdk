...
class ApplicationCredentials(BaseModel):
    ...
    """ # noqa: E501
    application_environment: StrictStr = Field(description="Specifies the application environment type such as kOracle, kSQL, etc. running on the Protection Source.", alias="applicationEnvironment")
    credentials: Optional[Credentials] = None
    __properties: ClassVar[List[str]] = ["applicationEnvironment", "credentials"]

    @field_validator('application_environment')
    def application_environment_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['kSQL', 'kOracle']):
            raise ValueError("must be one of enum values ('kSQL', 'kOracle')")
        return value
