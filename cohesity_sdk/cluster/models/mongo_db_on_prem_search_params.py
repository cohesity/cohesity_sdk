...
class MongoDbOnPremSearchParams(BaseModel):
    """
    Parameters required to search Mongo DB on a cluster.
    """ # noqa: E501
    mongo_db_object_types: List[StrictStr] = Field(description="Specifies one or more MongoDB object types be searched.", alias="mongoDBObjectTypes")
    search_string: Optional[StrictStr] = Field(description="Specifies the search string to search the MongoDB Objects", alias="searchString")
    ...
    __properties: ClassVar[List[str]] = ["mongoDBObjectTypes", "searchString", "sourceIds"]

    @field_validator('mongo_db_object_types')
    def mongo_db_object_types_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['MongoDatabases', 'MongoCollections']):
                raise ValueError("each list item must be one of ('MongoDatabases', 'MongoCollections')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "mongoDBObjectTypes": obj.get("mongoDBObjectTypes"),
