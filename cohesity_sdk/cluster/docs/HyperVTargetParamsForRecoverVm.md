...
**overwrite_existing_vms** | **bool** | Specifies whether to overwrite existing VMs while performing recovery of a VM. Default value is false. | [optional] 
...
**use_smb_service** | **bool** | Specifies if the HyperV recovery is using the SMB service to perform the restore. It is false for copy-recovery if nothing is specified. For instant-recovery it is true by default. Since DMaaS does not support SMB, only stream copy recovery is supported. | [optional] 
