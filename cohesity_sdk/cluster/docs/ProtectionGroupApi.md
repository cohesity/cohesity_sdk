...
> delete_protection_group(id, delete_snapshots=delete_snapshots, reason=reason)
...
with cohesity_sdk.cluster.ApiClient(configuration) as api_client:
    ...
    reason = 'reason_example' # str | Specifies the reason for group deletion with or without snapshots (optional)
 ...
 **reason** | **str**| Specifies the reason for group deletion with or without snapshots | [optional] 
...
with cohesity_sdk.cluster.ApiClient(configuration) as api_client:
    ...
    truncate_response = True # bool | If set, magneto will truncate the response if it exceeds max size limit governed by magneto_http_rpc_response_size_limit_bytes (optional)
    only_return_shell_info = True # bool | If set, returns only shell info such as run's start time, type, error if any. (optional)
    exclude_error_runs = True # bool | Specifies whether to exclude runs with error. If no value is specified, then runs with errors are included. (optional)
    job_run_start_time_usecs = 56 # int | Return a specific Job Run by specifying a time and a group id. Specify the time when the Job Run started as a Unix epoch Timestamp (in microseconds). If this field is specified, jobId must also be specified. (optional)
    only_return_data_migration_jobs = True # bool | Specifies if only data stubbing jobs should be returned. If not set, no data migration job will be returned. (optional)
    include_extension_info = True # bool | Specifies if needs to include proto extensions if they are extended. (optional)
    include_rpo_snapshots = True # bool | If true, then the snapshots for Protection Sources protected by Rpo policies will also be returned. (optional)
    source_id = 56 # int | Filter by source id. Only Job Runs protecting the specified source (such as a VM or View) are returned. The source id is assigned by the Cohesity Cluster. (optional)
 ...
 **truncate_response** | **bool**| If set, magneto will truncate the response if it exceeds max size limit governed by magneto_http_rpc_response_size_limit_bytes | [optional] 
 **only_return_shell_info** | **bool**| If set, returns only shell info such as run&#39;s start time, type, error if any. | [optional] 
 **exclude_error_runs** | **bool**| Specifies whether to exclude runs with error. If no value is specified, then runs with errors are included. | [optional] 
 **job_run_start_time_usecs** | **int**| Return a specific Job Run by specifying a time and a group id. Specify the time when the Job Run started as a Unix epoch Timestamp (in microseconds). If this field is specified, jobId must also be specified. | [optional] 
 **only_return_data_migration_jobs** | **bool**| Specifies if only data stubbing jobs should be returned. If not set, no data migration job will be returned. | [optional] 
 **include_extension_info** | **bool**| Specifies if needs to include proto extensions if they are extended. | [optional] 
 **include_rpo_snapshots** | **bool**| If true, then the snapshots for Protection Sources protected by Rpo policies will also be returned. | [optional] 
