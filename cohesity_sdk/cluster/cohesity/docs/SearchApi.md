...
from cohesity_sdk.cluster.api import search
from cohesity_sdk.cluster.cohesity.model.search_indexed_objects_request import SearchIndexedObjectsRequest
from cohesity_sdk.cluster.cohesity.model.search_indexed_objects_response_body import SearchIndexedObjectsResponseBody
...
import cohesity_sdk.cluster
from cohesity_sdk.cluster.api import search
from cohesity_sdk.cluster.cohesity.model.objects_search_response_body import ObjectsSearchResponseBody
from cohesity_sdk.cluster.cohesity.model.error import Error
from pprint import pprint
# Defining the host is optional and defaults to http://localhost/v2
# See configuration.py for a list of all supported configuration parameters.
...
with cohesity_sdk.cluster.ApiClient(configuration) as api_client:
    ...
    aws_object_types = [
        "kIAMUser",
    ] # [str] | Specifies the object types to filter objects on. Only applicable if the environment is AWS. (optional)
    azure_uuids = [
        "azureUuids_example",
    ] # [str] | Specifies the Azure UUID for the Microsoft365 objects. If specified, the objects with the matching Azure UUIDs will be returned. (optional)
    source_ids = [
        1,
    ...
    tag_categories = [
        "Security",
    ] # [str] | Specifies the tag category to filter the objects and snapshots. (optional)
    tag_sub_categories = [
        "Classification",
    ] # [str] | Specifies the tag subcategory to filter the objects and snapshots (optional)
    ...
    external_filters = [
        "externalFilters_example",
    ] # [str] | Specifies the key-value pairs to filtering the results for the search. Each filter is of the form 'key:value'. The filter 'externalFilters:k1:v1&externalFilters:k2:v2&externalFilters:k2:v3' returns the documents where each document will match the query (k1=v1) AND (k2=v2 OR k2 = v3). Allowed keys: - vmBiosUuid - graphUuid - arn - instanceId - bucketName - azureId (optional)
 ...
 **aws_object_types** | **[str]**| Specifies the object types to filter objects on. Only applicable if the environment is AWS. | [optional]
 ...
 **external_filters** | **[str]**| Specifies the key-value pairs to filtering the results for the search. Each filter is of the form &#39;key:value&#39;. The filter &#39;externalFilters:k1:v1&amp;externalFilters:k2:v2&amp;externalFilters:k2:v3&#39; returns the documents where each document will match the query (k1&#x3D;v1) AND (k2&#x3D;v2 OR k2 &#x3D; v3). Allowed keys: - vmBiosUuid - graphUuid - arn - instanceId - bucketName - azureId | [optional]
...
import cohesity_sdk.cluster
from cohesity_sdk.cluster.api import search
from cohesity_sdk.cluster.cohesity.model.protected_objects_search_response_body import ProtectedObjectsSearchResponseBody
from cohesity_sdk.cluster.cohesity.model.error import Error
from pprint import pprint
# Defining the host is optional and defaults to http://localhost/v2
# See configuration.py for a list of all supported configuration parameters.
