...
------------ | ------------- | ------------- | -------------
**objects** | [**[NutanixFSProtectionGroupObjectParams]**](NutanixFSProtectionGroupObjectParams.md) | Specifies the objects to be included in the Protection Group. | 
**backup_existing_snapshot** | **bool, none_type** | Specifies that snapshot label is not set for Data-Protect Nutanix FS Volumes backup. If field is set to true, existing oldest snapshot is used for backup and subsequent incremental will be selected in ascending order of snapshot create time on the source. If snapshot label is set, this field is set to false. | [optional] 
...
**modify_source_permissions** | **bool, none_type** | Specifies if the Nutanix FS source permissions should be modified internally to allow backups. | [optional] 
