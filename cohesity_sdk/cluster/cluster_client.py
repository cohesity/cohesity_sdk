...
from cohesity_sdk.cluster.model.create_access_token_request_params import (
...
class ClusterClient:
    def __init__(
        ...
        if username == None and password == None and api_key == None:
            raise Exception(
                "Missing authentication info to initialize a client. \
    ...
    def __get_token(self):
        ...
        with ApiClient(self.configuration) as api_client:
            ...
            body = CreateAccessTokenRequestParams(
                domain=self.domain, password=self.password, username=self.username
            )

            try:
                return api_instance.create_access_token(
                    body, _request_timeout=self.auth_timeout
                )
    ...
    def __authenticate(self):
        if self.username and self.password and self.domain:
            ...
            self.configuration.api_key["TokenHeader"] = (
